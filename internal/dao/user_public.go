...
func (u *UserDao) RegisterUserWithNewCompany(user *models.User, club *models.Club) error {
	tx := u.db.Begin()
	defer func() {
		if r := recover(); r != nil {
			tx.Rollback()
...
func (u *UserDao) RegisterUserWithExistingCompany(user *models.User, club *models.Club) error {
	tx := u.db.Begin()
	defer func() {
		if r := recover(); r != nil {
			tx.Rollback()
...
func (u *UserDao) RegisterUserWithToken(user *models.User) error {
	tx := u.db.Begin()
	defer func() {
		if r := recover(); r != nil {
			tx.Rollback()
